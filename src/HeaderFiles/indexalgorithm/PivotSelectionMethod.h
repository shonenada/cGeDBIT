...
public:
	...
	*/
	virtual vector<int> selectPivots(CMetric *metric,vector<shared_ptr<CIndexObject> > &data,int first,int size,int numPivots)=0;
	...
	/**@brief select the pivot according to the given parameters
		...
		@return return a container consists of the indexes of the selected pivots in the given data set	*/
	virtual vector<int> selectPivots(CMetric *metric, vector<shared_ptr<CIndexObject> > &data,int numPivots) = 0;
