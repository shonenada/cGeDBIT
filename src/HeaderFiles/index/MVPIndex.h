...
*/
	CMVPIndex(vector<shared_ptr<CIndexObject> > &dataObjects,CMetric *metric,CPivotSelectionMethod* psm,CPartitionMethod* pm,int numPivot,int singlePivotFanout,int maxLeafSize);
    ...
    void bulkLoad(vector<shared_ptr<CIndexObject> >& data,int runMode);
	...
	*/
    ...
    vector<shared_ptr<CIndexObject> > travelSearch(CQuery* q);
    ...
    void loadLeafToRoot(vector<shared_ptr<CIndexObject> > &dataObjects);

    void loadRootToLeaf(vector<shared_ptr<CIndexObject> > &dataObjects);
