...
/** @addtogroup CPeptideMetric
 *  @{
 * This class define a metric to compute distance of two peptide.
 */

/** A edit distance matrix of each two amino acid. */
double CPeptideMetric::mPAM250aExtendedWeightMatrix[AMINOACIDNUMBER-2][AMINOACIDNUMBER-2] = 
{
...
double CPeptideMetric::getDistance(CPeptide* one, CPeptide* two)
{
	...
	double dist = 0.0;
	vector<int> one_byte = one->getAminoAcidIDs();
	vector<int> two_byte = two->getAminoAcidIDs();
	for(i=0;i<one_size;i++){
		dist = dist + CPeptideMetric::mPAM250aExtendedWeightMatrix[one_byte[i]][two_byte[i]];
	}
	return dist;
}
