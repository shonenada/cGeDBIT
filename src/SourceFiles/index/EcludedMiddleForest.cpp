...
CIndexNode* CEcludedMiddleForest::createRoot(ForestTask *forestTask,vector<CIndexObject*>& ecludedData)
{
...
void CEcludedMiddleForest::createInternalNode(ForestTask *forestTask,vector<CIndexObject*>& ecludedData)
{
    this->numInternalNodes ++;
...
void CEcludedMiddleForest::createLeafNode(ForestTask *forestTask,vector<CIndexObject*>& ecludedData)
{
    this->numLeaf++;
...
vector<CIndexObject*> CEcludedMiddleForest::createTree(vector<CIndexObject*> &dataObjects,int treeIndex)
{
    ...
    while(this->forestTaskList.size()>0)
    {
        ...
        if(roots.size()<treeIndex+1)
        {
            ...
            this->roots.push_back(createRoot(forestTask,ecludedData));
        }
        else
        {
            /**get the number of pivot*/
            ...
            if(remainNodeSize>maxLeafSize)
                createInternalNode(forestTask,ecludedData);
            else
                createLeafNode(forestTask,ecludedData);
        }
...
void CEcludedMiddleForest::bulkLoad(vector<CIndexObject*> & dataList,int runMode)
{
