...
private:
    /**constructor*/
    ForestTask():dataList(*(new vector<shared_ptr<CIndexObject> >))
    {
        this->parentIndex=0;
        this->myIndex=0;
        this->myHeight=0;
    ...
    */
    ForestTask(vector<shared_ptr<CIndexObject> > &dataList,shared_ptr<CIndexNode> parentIndex,int start,int end,int myIndex,int myHeight):dataList(dataList)
    {
...
* @param pivots a vector to store the address of pivot.This vector is initialized with none elements, all the elements is generated through calling thing function. This parameter is used to return the address of all pivot to the caller of this funciton.
*/
void ForestTask::getPivots(vector<shared_ptr<CIndexObject> > &pivots)
{
...
/**get all the objects that belong to the tree rooted at this node*/
void ForestTask::getDataPoints(vector<shared_ptr<CIndexObject> > &objectList)
{
    int mySize=0;
...
shared_ptr<CIndexNode> CExcludedMiddleForest::createRoot(shared_ptr<ForestTask> forestTask,vector<shared_ptr<CIndexObject> >& ecludedData)
{
    ...
    {
        shared_ptr< ForestTask> newForestTask = 0;
...
void CExcludedMiddleForest::createInternalNode(shared_ptr<ForestTask> forestTask,vector< shared_ptr<CIndexObject> >& ecludedData)
{
    this->numInternalNodes ++;
...
void CExcludedMiddleForest::createLeafNode(shared_ptr< ForestTask> forestTask,vector<shared_ptr<CIndexObject> >& ecludedData)
{
    this->numLeaf++;
...
vector<shared_ptr<CIndexObject> > CExcludedMiddleForest::createTree(vector<shared_ptr< CIndexObject> > &dataObjects,int treeIndex)
{
    vector<shared_ptr< CIndexObject> > ecludedData;
    ofstream out;
...
void CExcludedMiddleForest::bulkLoad(vector<shared_ptr<CIndexObject> > & dataList,int runMode)
{
    ...
    vector<shared_ptr<CIndexObject> > formerEcludedData = createTree(dataList,0);
    vector<shared_ptr< CIndexObject> >* ecludedData;
...
vector<shared_ptr< CIndexObject> >* CExcludedMiddleForest:: search(CQuery* q)
{
...
CExcludedMiddleForest::CExcludedMiddleForest(vector<shared_ptr<CIndexObject> > &dataObjects,CMetric *metric,CPivotSelectionMethod* psm,CPartitionMethod* pm,int numPivot,int singlePivotFanout,int maxLeafSize, int maxPathLength,double maxRadius,double middlePorprotion)
{
...
void CExcludedMiddleForest::setMetric(CMetric* metric )
{
    this->metric=metric;
}

void CExcludedMiddleForest::writeRoot(shared_ptr <ForestTask> task,ofstream& out,vector<shared_ptr<CIndexObject> >& ecludedData, int treeIndex)
{ 
    out.open(fileName,ios::binary|ios::_Nocreate);
...
void CExcludedMiddleForest::createAndWriteInternalNode(shared_ptr <ForestTask> task,ofstream& out,vector<shared_ptr<ForestTask> >::iterator &shouldeBeCreate,vector<shared_ptr<CIndexObject> >& ecludedData)
{
    this->numInternalNodes ++;
...
void CExcludedMiddleForest::createAndWriteLeafNodeLast(shared_ptr <ForestTask> task,ofstream &out,vector<shared_ptr<CIndexObject> >& ecludedData,int treeIndex)
{
...
void CExcludedMiddleForest::loadRootToLeaf(vector<shared_ptr<CIndexObject> > &dataObjects,vector<shared_ptr<CIndexObject> >& ecludedData,int treeIndex,ofstream &out)
{      
    ...
    shared_ptr<ForestTask> task(new ForestTask(dataObjects,NULL,0,dataObjects.size(),0,0));
