...
struct IndexObjectComparer
{
	...
	*/
	bool operator() (shared_ptr<CIndexObject> first, shared_ptr<CIndexObject> second)
	{
...
/**
* @brief    Sort vector of CIndexObject using std::sort method.
* @param vectorToBeSorted       A vector of CIndexObject waiting to be sorted.
...
*/
void sortVector(vector<shared_ptr<CIndexObject> > &vectorToBeSorted, shared_ptr<CIndexObject> pivot, CMetric* metric, int begin, int size)
{
	IndexObjectComparer comparer;
	comparer.metric = metric;
	comparer.pivot = pivot;

	vector<shared_ptr<CIndexObject> >::iterator startIndex = vectorToBeSorted.begin() + begin;
	vector<shared_ptr<CIndexObject> >::iterator endIndex = vectorToBeSorted.begin() + size;
...
*/
CPartitionResults CBalancePartitionMethod::partition(CMetric *metric,
													 const vector<shared_ptr<CIndexObject> > &pivots,
													 vector<shared_ptr<CIndexObject> > &data, int first,
													 int size, double maxRadius, int maxPartitions,
													 int maxLeafSize, double middleProportion)
{
	return partition(metric,  pivots , data, first,  size,  maxPartitions, maxLeafSize);
}
...
* @return      A CPartitionResults object which pack the partition results together.
*/
CPartitionResults CBalancePartitionMethod::partition(CMetric *metric, const vector<shared_ptr<CIndexObject> > &pivots, 
													 vector<shared_ptr<CIndexObject> > &data, int first, int size,
													 int maxPartitions, int maxLeafSize)
{
