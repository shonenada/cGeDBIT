 ...
 * @return 
 */
vector<int> removeDuplicate(CMetric *metric,vector<shared_ptr<CIndexObject> > data,vector<int> &pivots,int first,int dataSize);
...
vector<int> CFFTPivotSelectionMethod::selectPivots(CMetric *metric, vector<shared_ptr<CIndexObject> > &data,int numPivots)
{
	 return selectPivots(metric, data,0,data.size(),numPivots);
}

vector<int> CFFTPivotSelectionMethod::selectPivots(CMetric *metric, vector<shared_ptr<CIndexObject> >& data, int first, int dataSize, int numPivots)
{
...
vector<int> CFFTPivotSelectionMethod::selectPivots(CMetric *metric, vector<shared_ptr<CIndexObject> > &data,int first, int dataSize, int numPivots, int firstPivot)
{
	...
	for (int i = 0; i < dataSize; i++)
	{
	 ...
	 for (int centerSize = 1; centerSize < numPivots; centerSize++)
	 {
                ...
                shared_ptr<CIndexObject> const lastCenter = data[indices[centerSize - 1]];
