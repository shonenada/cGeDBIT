#include "../../HeaderFiles/indexalgorithm/ExcludedMiddlePartitionMethod.h"



void extract(vector<shared_ptr<CIndexObject> > &data,vector<double> &distance,vector<int> &offsets,int start,int target,int first,int type)
{
    shared_ptr<CIndexObject> tempObject1;
    shared_ptr<CIndexObject> tempObject2;
...
CPartitionResults CExcludedMiddlePartitionMethod:: partition(CMetric *metric, const vector<shared_ptr<CIndexObject> > &pivots,vector<shared_ptr<CIndexObject> > &data, int first, int size, int numPartitions, int maxLeafSize)
{
    ...
    return partition(metric, pivots,data, first,size,_maxRadius,numPartitions,maxLeafSize,_middleProportion);
}
